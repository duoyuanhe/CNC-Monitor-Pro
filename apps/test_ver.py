    ...
    if by_cell:
        ...
        if cell_number:
            ...
            for cell in cell_number:
                ...
                data_each_cell = data_in_duration[mask_cell]
                data_by_cell.append(data_each_cell)
            ...
            if point_or_customization:
                ...
                if point_or_customization == 'Fixed':
                    ...
                    if number:
                        ...
                        filter_FAI = [
                            col for col in data_in_duration if col.startswith(FAI_number)]
                        violin_or_box = st.sidebar.select_slider('Box/Violin', ['box', 'violin'])
                        if filter_FAI and violin_or_box:
                            for i, data_in_duration in enumerate(data_by_cell):
                                data = data_in_duration[filter_FAI]
                                filtered_data.append(data)
                                # st.markdown('### ' + violin_or_box.capitalize() +
                                #             'plot of  ' + cell_number[i])
                                st.markdown('### ' + cell_number[i])
                                fig, ax = plt.subplots()
                ...
                if review_by_day and mode:
                    ...
                    if ready:
                        for i, data in enumerate(filtered_data):
                            st.markdown('## ' + cell_number[i])
                            df_union, length = get_df_at_different_points_duration(
                                data, precision[0])
